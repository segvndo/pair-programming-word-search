//Swap rows with columns via transpose
const transpose = (matrix) => {
    return matrix[0].map((_, colIndex) =>
        matrix.map((row) =>[colIndex])
    );
  };
  ...
  const wordSearch = (letters, word) => {
    ...
    //Where diagonal strings will be pushed into
    ...
    //Concat all generated strings into one array
    const allStrings = horizontalStrings
        .concat(verticalStrings)
        .concat(diagonalStrings);
  
    //Check if word or its reverse is found in any string
